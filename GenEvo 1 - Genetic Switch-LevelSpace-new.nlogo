...
LacIs-own [
  partner                ;; partner is a lactose molecule with which LacI forms a complex
...
[ view-width       ;; global veriables to manage view and cell-dimensions (these are only for display purpose)
  ...
  ini-energy          ;; initial energy of the cell
  lactose-quantity
  division-number
...
to setup
  ...
  add-proteins
  if not LevelSpace? [
    if lactose? [add-lactose lactose-quantity]
  ]
  create-var-list
  reset-ticks
end
...
;; procedure that assigns a specific shape to an agent, and shows
...
to go
  ...
  if not LevelSpace? [
    update-lactose lactose-quantity
    divide-cell
    if (energy < 0) [
...
to update-energy
  set energy (energy - floor ( count RNAPs + count LacIs + count LacZs) / 20 )
...
to go-LacIs
  ask LacIs [
    ifelse bound-to-operator? [
       ...
       if (count lactoses-here with [partner = nobody] > 0 ) [ bind-to-lactose ]
    ]
    [
      move
      if (inhibited? = false) and ((member? patch-here operator) or (member? patch-ahead 1 operator)) and (partner = nobody) [ bind-to-operator ]
      if (partner = nobody) and (count lactoses-here with [partner = nobody] > 0 ) [ bind-to-lactose ]
    ]
    setshape
  ]
end
...
to bind-to-lactose
  if (random-float 1 < lacI-lactose-binding-chance) [
...
to go-RNAPs
  ...
  if (not inhibited?) [
    ask RNAPs [
      if not glucose? [
        if (member? patch-here promoter) or (member? patch-ahead 1 promoter) or (member? patch-ahead 2 promoter)[
          bind-to-promoter
        ]
      ]
    ]
...
to create-proteins
    ...
    ;; Energy calculations - synthesis of one LacY or LacZ reduces energy by energy units equal to their production cost
...
to move
  if (breed != LacYs and breed != LacZs) and (xcor > (cell-width - 3) or xcor < (- cell-width + 3)) [
    genXY-inside
  ]
...
to divide-cell
  if (energy > 2 * ini-energy) [
    ...
    let half-LacZ-inside-count count LacZs / 2
    ask n-of half-LacZ-inside-count LacZs [die]
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;; Independant/LevelSpace Procedures ;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

to update-lactose [num-lactose]
  ifelse LevelSpace? [
    let lactose-outside count lactoses with [not inside?]
    ifelse (lactose-outside >= num-lactose) [
      ask n-of (lactose-outside - num-lactose) lactoses with [not inside?] [die]
    ]
    [
      add-lactose ( num-lactose - lactose-outside )
    ]
  ]
  [
    if (lactose?) [
      add-lactose ( num-lactose - count lactoses with [ inside? = false] )
    ]
  ]
end

;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;; Exculsive LevelSpace Procedures ;;;;;;;;;;;;;;;;;
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;

to make-var-name-list
  set var-name-list ["energy" "LacYs" "lacY-installed" "LacZs" "lacI-lacotose-complex" "lactose-inside" "lactose-outside"]
end

to create-var-list
  set var-list (list (energy) (count LacYs) (count (patches with [pcolor = orange])) (count LacZs)
      (count (LacIs with [partner != nobody])) (count (lactoses with [inside?])) (count (lactoses with [not inside?])))
end
...
910
545
Average cell division time (Growth Rate Indicator)
NIL
NIL
0.0
10.0
...
510
Cell Division Time
precision cell-division-time 0
17
1
11
